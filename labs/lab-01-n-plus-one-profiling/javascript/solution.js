// Lab 01: N+1 Query Profiling — JavaScript reference solution (PostgreSQL).
//
// Runs against real Postgres through labkit; db.queryCount proves 101 -> 1 -> 2.
...
async function fetchInBatch() {
  ...
  return users.map((u) => ({ id: u.id, name: u.name, posts: byUser.get(u.id) }));
}

// Part 2 — DataLoader: N load() calls within one tick coalesce into 1 query.
