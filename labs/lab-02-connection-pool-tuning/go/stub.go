// Lab 02: Connection Pool Tuning — Go, YOUR TURN.
//
// Implement Pool.Acquire and Pool.Release so the pool:
//   - reuses idle connections,
//   - blocks up to `timeout` when exhausted (then returns errPoolExhausted),
//   - never creates more than maxSize connections.
//
//	Run / validate:  go run stub.go     (the embedded checks must pass)
//	Reference:        go run solution.go
package main

import (
	"errors"
	"fmt"
	"sync"
	"time"
)

const (
	connectionCreationCost = 15 * time.Millisecond
	queryCost              = 10 * time.Millisecond
)

var errPoolExhausted = errors.New("pool exhausted: acquire timed out")

type Conn struct{ ID int }

func (c *Conn) Execute() { time.Sleep(queryCost) }

type Pool struct {
	maxSize int
	timeout time.Duration
	sem     chan struct{}
	mu      sync.Mutex
	idle    []*Conn
	created int
}

func NewPool(maxSize int, timeout time.Duration) *Pool {
	return &Pool{maxSize: maxSize, timeout: timeout, sem: make(chan struct{}, maxSize)}
}

// Acquire — TODO.
//
//	1. Take a permit: `select` on `p.sem <- struct{}{}` vs `time.After(p.timeout)`;
//	   on timeout return nil, errPoolExhausted.
//	2. Under p.mu: pop an idle connection if any.
//	3. Otherwise increment p.created, capture the id, unlock, sleep
//	   connectionCreationCost, and return &Conn{ID: id}.
func (p *Pool) Acquire() (*Conn, error) {
	panic("TODO: implement Pool.Acquire")
}

// Release — TODO.
//
//	1. Under p.mu: append c to p.idle.
//	2. Free one permit: `<-p.sem`.
func (p *Pool) Release(c *Conn) {
	panic("TODO: implement Pool.Release")
}

func (p *Pool) Created() int {
	p.mu.Lock()
	defer p.mu.Unlock()
	return p.created
}

func assert(cond bool, msg string) {
	if !cond {
		panic("CHECK FAILED: " + msg)
	}
}

func main() {
	p := NewPool(2, 5*time.Second)
	c1, _ := p.Acquire()
	p.Release(c1)
	c2, _ := p.Acquire()
	assert(c1.ID == c2.ID, "released connection should be reused")
	p.Release(c2)

	p2 := NewPool(1, 200*time.Millisecond)
	held, _ := p2.Acquire()
	_, err := p2.Acquire()
	assert(errors.Is(err, errPoolExhausted), "second acquire should time out")
	p2.Release(held)

	p3 := NewPool(10, 30*time.Second)
	var wg sync.WaitGroup
	var ok int
	var okMu sync.Mutex
	for i := 0; i < 100; i++ {
		wg.Add(1)
		go func() {
			defer wg.Done()
			c, err := p3.Acquire()
			if err != nil {
				return
			}
			c.Execute()
			p3.Release(c)
			okMu.Lock()
			ok++
			okMu.Unlock()
		}()
	}
	wg.Wait()
	assert(ok == 100, "all 100 requests should succeed")
	assert(p3.Created() <= 10, "pool must never create more than maxSize connections")

	fmt.Printf("OK — reuse, timeout, and bound (created=%d for 100 requests, max=10)\n", p3.Created())
}
